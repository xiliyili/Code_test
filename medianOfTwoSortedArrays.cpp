...
double findMedianSortedArrayHelper(int A[], int m, int B[], int n, int lowA, 
int highA, int lowB, int highB) {
    ...
    if (num == (m+n)/2){
        ...
        // You can see, the `pos` is the place A[mid] can be inserted, so 
return A[mid]
        if ((m+n)%2==1){
            return A[mid];
        }
    ...
    if (num < (m+n)/2){
        ...
        if ( highA - lowA > highB - lowB ) {
            return findMedianSortedArrayHelper(A, m, B, n, lowA, highA, lowB, 
highB);
        }
        return findMedianSortedArrayHelper(B, n, A, m, lowB, highB, lowA, 
highA);
    }
...
double findMedianSortedArrays(int A[], int m, int B[], int n) {
    ...
    //if the length of array is even, return the average of the middle two 
numbers
